...
export default function WisePaperPage() {
	...
	const flowSteps = [
		{ key: "import", icon: Upload, number: 1 },
		{ key: "process", icon: Sparkles, number: 2 },
		{ key: "track", icon: LineChart, number: 3 },
	];
						...
						<div className="relative bg-card border border-border rounded-xl overflow-hidden shadow-2xl">
							<div className="flex items-center gap-2 px-4 py-3 bg-muted/50 border-b border-border">
								...
								<div className="flex-1 text-center">
									<span className="text-xs text-muted-foreground">WisePaper - Formation digitale</span>
								</div>
							</div>
			...
			>
				<div className="max-w-4xl mx-auto">
					...
					<div className="hidden md:block">
						<div className="relative">
							{/* Connecting line */}
							<div className="absolute top-6 left-[16%] right-[16%] h-0.5 bg-secondary/30" />
							<div className="absolute top-[22px] left-[16%] right-[16%] h-1 bg-gradient-to-r from-secondary/60 via-secondary to-secondary/60 rounded-full" />
										...
										<div key={step.key} className="relative flex flex-col items-center text-center">
											...
											<div className="mt-3 mb-2">
												<span className="inline-block text-xs font-semibold text-secondary bg-secondary/10 border border-secondary/20 px-3 py-1 rounded-full">
													{t(`flow.steps.${step.key}.duration`)}
												</span>
											</div>
						...
						<thead>
							<tr className="border-b-2 border-border">
								...
								{productSolutions.map((solution) => {
									...
									const isCurrent = solution.key === "wisepaper";
									return (
										<th
											key={solution.key}
											className={cn(
